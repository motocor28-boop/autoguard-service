/* EECR — seis registros demostrativos con fotografías reales de uso libre. */
(()=>{
  'use strict';
  const STORE='mantpro-records-v3',LEGACY='mantpro-records',C=window.MANTPRO_CONFIG||{};
  ...
  const at=(hour,minute=0)=>{const now=new Date(),d=new Date(now.getFullYear(),now.getMonth(),now.getDate(),hour,minute,0,0);return d.toISOString()};
  ...
  async function seed({navigate=true}={}){
    const map=new Map(parse(STORE).map(x=>[x.id,x]));examples().forEach(x=>map.set(x.id,x));localStorage.setItem(STORE,JSON.stringify([...map.values()]));
    window.dispatchEvent(new CustomEvent('mantpro-records-changed',{detail:{demoLoaded:true,count:IDS.length}}));
    try{await window.MANTPRO?.sync?.()}catch(error){console.warn('EECR demo sync:',error)}
    notify('Se cargaron 6 registros demostrativos con fotografías y descripciones.');
    if(navigate)setTimeout(()=>{$('[data-route="reports"]')?.click();setTimeout(()=>{const date=$('#ehs-report-date');if(date){date.value=localDate();date.dispatchEvent(new Event('change',{bubbles:true}))}},250)},500);
    return true;
  }
  async function remove(){
    ...
    if(token&&navigator.onLine&&C.supabaseUrl){
      for(const id of IDS){const response=await fetch(`${C.supabaseUrl}/rest/v1/mantpro_records?id=eq.${encodeURIComponent(id)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}});if(!response.ok)throw new Error(`No fue posible eliminar ${id} de la nube.`)}
    }
    localStorage.setItem(STORE,JSON.stringify(parse(STORE).filter(x=>!IDS.includes(x.id))));
    localStorage.setItem(LEGACY,JSON.stringify(parse(LEGACY).filter(x=>!IDS.includes(x.id))));
    window.dispatchEvent(new CustomEvent('mantpro-records-changed',{detail:{demoRemoved:true}}));
    window.MANTPRO?.render?.();notify('Los registros demostrativos fueron eliminados.');return true;
  }
  function panel(){
    const app=$('#app');if(!app||!/^Informes$/i.test(app.querySelector('h1')?.textContent?.trim()||''))return;
    let box=$('#eecr-demo-panel',app);if(!box){box=document.createElement('section');box.id='eecr-demo-panel';box.className='callout section';box.innerHTML='<div><b>Demostración del informe final</b><br><small>Carga seis registros identificados con fotografías y descripciones. Se pueden eliminar después sin afectar los datos reales.</small></div><div class="button-row"><button type="button" class="primary" data-eecr-demo-load>Cargar 6 ejemplos</button><button type="button" class="outline" data-eecr-demo-remove>Eliminar ejemplos</button></div><p data-eecr-demo-state class="muted"></p>';app.querySelector('h1')?.insertAdjacentElement('afterend',box);box.querySelector('[data-eecr-demo-load]').onclick=()=>seed();box.querySelector('[data-eecr-demo-remove]').onclick=async()=>{try{await remove()}catch(error){alert(error.message||'No fue posible eliminar la demostración.')}}}
    const state=box.querySelector('[data-eecr-demo-state]');if(state)state.textContent=`Registros demostrativos cargados: ${count()} de 6.`;
  }
  let timer;const enhance=()=>{clearTimeout(timer);timer=setTimeout(panel,90)};
  const start=()=>{new MutationObserver(enhance).observe($('#app')||document.body,{childList:true,subtree:true});window.addEventListener('mantpro-records-changed',enhance);enhance();const params=new URLSearchParams(location.search);if(params.get('demo')==='6'){setTimeout(()=>seed(),1000);params.delete('demo');const query=params.toString();history.replaceState({},'',location.pathname+(query?'?'+query:'')+location.hash)}};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start);else start();
  window.EECR_DEMO={seed,remove,count,ids:IDS,build:'2026-08-02-demo-six'};
})();
